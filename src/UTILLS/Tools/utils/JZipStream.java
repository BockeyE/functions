 ...
 */
public class JZipStream extends DeflaterOutputStream {
    ...
    private static class XEntry {
        ...
        public XEntry(ZipEntry entry, long offset) {
            this.entry = entry;
            this.offset = offset;
            this.flag = (entry.getMethod() == DEFLATED && (entry.getSize() == -1 || entry.getCompressedSize() == -1 || entry
                    .getCrc() == -1))
     ...
     */
    public JZipStream(OutputStream out) {
        super(out, new Deflater(Deflater.DEFAULT_COMPRESSION, true));
        try {
            Field field = DeflaterOutputStream.class.getDeclaredField("usesDefaultDeflater");
            if (field != null) {
            ...
            case DEFLATED:
                ...
                if ((current.flag & 8) == 0) {
                    ...
                    if (e.getSize() != def.getBytesRead()) {
                        throw new ZipException("invalid entry size (expected " + e.getSize() + " but got "
                                + def.getBytesRead() + " bytes)");
                    }
                    if (e.getCompressedSize() != def.getBytesWritten()) {
                        throw new ZipException("invalid entry compressed size (expected " + e.getCompressedSize()
                                + " but got " + def.getBytesWritten() + " bytes)");
                    }
                    if (e.getCrc() != crc.getValue()) {
                        throw new ZipException("invalid entry CRC-32 (expected 0x" + Long.toHexString(e.getCrc())
                ...
                break;
            case STORED:
                // we already know that both e.getSize()and e.csize are the same
    ...
    public static void main(String[] args) throws IOException {
        JZipStream zos = new JZipStream(new FileOutputStream("d:/1.zip"));
        try {
            ZipEntry ze = new ZipEntry("abc123.txt");
            zos.putNextEntry(ze);
            zos.write("你好顶顶顶顶顶顶顶顶".getBytes("gbk"));
            zos.closeEntry();
            ZipEntry z2e = new ZipEntry("顶.txt");
            zos.putNextEntry(z2e);
            zos.write("你好顶顶顶顶顶顶顶顶".getBytes("gbk"));
            zos.closeEntry();
        } finally {
            zos.close();
